    ...
    <div class="hentry hentry-post blog-big group ">
        ...
        @if($article)
            ...
            <div class="meta group">
                ...
                <p class="categories"><span>In: <a href="{{ route('articlesCategory',['alias'=>$article->category->alias]) }}" title="View all posts in {{ $article->category->title }}" rel="category tag">{{ $article->category->title }}</a></p>
                <p class="comments"><span><a href="#comments" title="Comment on This is the title of the first article. Enjoy it.">{{ count($article->comments) ? count($article->comments) : '0' }} {{Lang::choice('blog.comments',count($article->comments))}}</a></span></p>
            </div>
            <!-- post content -->
